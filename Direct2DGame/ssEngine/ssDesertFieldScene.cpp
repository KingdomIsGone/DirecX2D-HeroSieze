...
namespace ss
{
	DesertFieldScene::DesertFieldScene()
	{
	}
	DesertFieldScene::~DesertFieldScene()
	{
	}
	void DesertFieldScene::Initialize()
	{
		std::shared_ptr<Shader> spriteShader = Resources::Find<Shader>(L"SpriteShader");
		{
			{
				std::shared_ptr<Texture> texture
					= Resources::Load<Texture>(L"DesertField", L"..\\Resources\\Texture\\Map\\Wormhole_2_rm.png");
				std::shared_ptr<Material> spriteMaterial = std::make_shared<Material>();
				spriteMaterial->SetShader(spriteShader);
				spriteMaterial->SetTexture(texture);
				Resources::Insert(L"DesertFieldMater", spriteMaterial);
			}
			GameObject* DesertField
				= object::Instantiate<GameObject>(Vector3(0.0f, 0.0f, 1.01f), eLayerType::Map);
			DesertField->SetName(L"DesertField");

			MeshRenderer* mr = DesertField->AddComponent<MeshRenderer>();
			mr->SetMesh(Resources::Find<Mesh>(L"RectMesh"));
			mr->SetMaterial(Resources::Find<Material>(L"DesertFieldMater"));
			//DesertField->GetComponent<Transform>()->SetPosition(Vector3(0.0f, 0.0f, 1.01f));
			DesertField->GetComponent<Transform>()->SetScale(Vector3(35.0f, 35.0f, 1.0f));
	...
	void DesertFieldScene::Update()
	{
		Scene::Update();
	}

	void DesertFieldScene::LateUpdate()
	{
