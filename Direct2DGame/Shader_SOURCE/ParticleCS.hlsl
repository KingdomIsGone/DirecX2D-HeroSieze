...
void main(uint3 DTid : SV_DispatchThreadID)
{
    ...
    if (ParticleBuffer[DTid.x].active == 0)
    {
        ...
        if (ParticleBuffer[DTid.x].active == 1)
        {
            ...
            ParticleBuffer[DTid.x].position.x = 1.0f;
            ParticleBuffer[DTid.x].position.y = 0.0f;
            ...
            //ParticleBuffer[DTid.x].endTime = vRandom.xyz * 5.0f;
            ParticleBuffer[DTid.x].endTime = 3.0f;
            ParticleBuffer[DTid.x].speed = vRandom.xyz * 3.0f;
            //ParticleBuffer[DTid.x].direction.x = vRandom.x;
